...
#引数から取得 実行例：python merscraping.py args
args = sys.argv
query = args[1]
...
url_site = "https://www.mercari.com/jp/search/?keyword={}&status_on_sale=1".format(query)
...
try: #エラーで途中終了しても途中までの分をcsvに書き出したいのでtry～except文
    for url in urls:
        ...
        #price = browser.find_element_by_xpath("//div[1]/section/div[1]/div[2]/span[1]").text #SP表示
